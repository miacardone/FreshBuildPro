...
export function ComplianceReportSheet({ project, evaluation }: DocProps) {
  ...
  return (
    ...
    >
      ...
      {(project.acknowledgements ?? []).length > 0 && (
        <Block title={`Acknowledged — proceeding despite ${project.acknowledgements!.length}`}>
          <p className="mb-2 text-[11px] text-ink-muted">
            These findings were read and a decision was made to proceed. They still count against
            readiness; acknowledging records the decision, it does not clear the finding.
          </p>
          <ul className="grid gap-2 text-[11.5px]">
            {project.acknowledgements!.map((a) => (
              <li key={`${a.ruleId}-${a.at}`} className="break-inside-avoid border-b border-dotted border-line pb-1.5">
                ...
                <div className="text-[10px] text-ink-muted">
                  Recorded {new Date(a.at).toLocaleString()}
                </div>
              </li>
            ))}
          </ul>
